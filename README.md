...
| `akzo_aging_pipeline.py` | Aging report: pulls SOA (GP_Lakehouse) + TMW 712 + IB detail, builds the combined aging workbook and the dashboard-accurate Maria report, and emails it through classic Outlook (COM): attaches to a running classic Outlook or starts one, then confirms the Outbox drained — no IT setup needed. Flags: `--display` (draft only), `--no-email`. |

## How the aging report emails (no IT setup)

`akzo_aging_pipeline.py` sends the Maria report through **classic Outlook via
COM automation** — the only mail path that needs nothing from IT. (New Outlook
/ olk.exe has no automation interface, Microsoft Graph is blocked for
unregistered sign-in clients, and SMTP basic auth is retired.) The script:

1. attaches to classic Outlook if it's already running,
2. otherwise starts classic `outlook.exe` itself and keeps retrying the
   attach for up to 2 minutes,
3. sends, forces a send/receive, and waits for the Outbox to drain before
   reporting **SENT** — otherwise it reports **QUEUED** honestly (the message
   goes out the next time classic Outlook is open and online).

You do **not** need to close new Outlook — classic and new Outlook can run at
the same time. Caveat: with the "new Outlook" toggle on (`UseNewOutlook=1`)
Windows may redirect even direct classic launches; if the script can't start
classic Outlook, open it manually (Start menu → *Outlook (classic)*), leave it
in the background, and rerun — the script attaches to the running instance.
...
to raw `Movement Type` and prints a warning.

Note: the interplant location list is a bid-event reference and has been updated
since the April close (col H now includes more distribution centers), so the
engine's interplant split reflects the current rule and may differ slightly from
the April manual figure.
