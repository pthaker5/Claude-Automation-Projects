...
def pull_boxing_materials(start, end, wh_filter=None):
    ...
    try:
        ...
        # passed twice.
        params = [start, end] + (list(wh_filter) if wh_filter else []) + (list(wh_filter) if wh_filter else [])
        return _q(conn, f"""
...
LEFT JOIN Korber.t_bmm_charge ch
    ON bel.order_number = ch.order_num_value
LEFT JOIN Korber.t_client c
